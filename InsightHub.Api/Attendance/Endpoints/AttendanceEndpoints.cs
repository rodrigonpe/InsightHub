...
public static class AttendanceEndpoints
{
    ...
    {  
        app.MapGet("/attendance/availability", async (
            DateTime? datetime,
            string? state,
            string? city,
            ...
            */
            ...
            await using (var exceptionCommand = new NpgsqlCommand(exceptionSql, connection))
            {
                ...
                if (await exceptionReader.ReadAsync())
                {
                    ...
                    var startTime = exceptionReader.IsDBNull(1)
                        ? (TimeOnly?)null
                        : TimeOnly.FromTimeSpan(exceptionReader.GetTimeSpan(1));

                    var endTime = exceptionReader.IsDBNull(2)
                        ? (TimeOnly?)null
                        : TimeOnly.FromTimeSpan(exceptionReader.GetTimeSpan(2));

                    var reason = exceptionReader.IsDBNull(3)
                        ? null
                    ...
                    return Results.Ok(new
                    {
                        ...
                        reason = available
                            ? null
                            : "OUTSIDE_SPECIAL_BUSINESS_HOURS",
                        description = reason
                    });
                }
            }
            ...
            /*
            Verifica fim de semana e feriados.
            Toda a lógica de feriados agora fica no HolidayService.
            */
            var isWeekend =
                currentDate.DayOfWeek == DayOfWeek.Saturday ||
                currentDate.DayOfWeek == DayOfWeek.Sunday;

            var holiday = await holidayService.FindHolidayAsync(
                currentDate,
                state,
                city);
            ...
            /*
            Se for dia útil e não houver exceção,
            aplica o horário padrão.
            */
            ...
            await using var businessHourCommand =
                new NpgsqlCommand(businessHourSql, connection);

            businessHourCommand.Parameters.AddWithValue(
                "dayOfWeek",
                dayOfWeek);

            await using var businessHourReader =
                await businessHourCommand.ExecuteReaderAsync();
            ...
            var defaultIsOpen =
                businessHourReader.GetBoolean(0);

            var defaultStartTime =
                businessHourReader.IsDBNull(1)
                    ? (TimeOnly?)null
                    : TimeOnly.FromTimeSpan(
                        businessHourReader.GetTimeSpan(1));

            var defaultEndTime =
                businessHourReader.IsDBNull(2)
                    ? (TimeOnly?)null
                    : TimeOnly.FromTimeSpan(
                        businessHourReader.GetTimeSpan(2));

            var defaultAvailable =
                defaultIsOpen &&
